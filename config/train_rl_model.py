...
class RewardModelConfig:
    ...
    reward_type: str = "raw" # raw | delta | discrete_raw | discrete_delta | majority_vote (delta, discrete_delta require initial student attempt generations)
    discrete_threshold: float = 0.5  # Only used for discrete reward types.
    vllm: ModelvLLMConfig = field(default_factory=ModelvLLMConfig)
...
class GenerationConfig:
    ...
    judges_rules_prompts_paths_original: Dict[str, str] = field(
        default_factory=lambda: {
    ...
    # Original Judge
    use_original_judge: bool = False  # If true, we use the original judge prompts and settings for reward computation.
    ...
    # Reward config
    extra_penalty_for_rejected_judges: float = 0.75

    # Turn-wise reward config (no-branch)
    pre_reject_weight_mode: str = "dynamic"  # "dynamic" | "fixed" ("strict" alias supported)
    pre_reject_weight_w: float = 1.0  # Legacy fallback when w1/w2 are not explicitly set.
    pre_reject_weight_w1: Optional[float] = None  # Accuracy component weight.
    pre_reject_weight_w2: Optional[float] = None  # think/eoc/length combined component weight.

    # If enabled, turn reward uses: (accuracy component) + (think/eoc/length component),
    # and pre-reject scaling can be applied with separate weights (w1, w2).
    use_turn_level_aux_rewards: bool = False
    turn_level_accuracy_weight: float = 1.0
    turn_level_think_weight: float = 1.0
    turn_level_eoc_weight: float = 1.0
    turn_level_length_weight: float = 1.0
    ...
    include_next_student_utterance: bool = False  # Whether to include the next student utterance in the judge context (for each turn).
...
class TrainConfig:
    ...
    use_turn_level_advantage: bool = True
    log_singleton_stats: bool = True
