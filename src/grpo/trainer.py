...
from src.vllm.client import get_batch_metrics, sample_conversations, wait_batch
...
class ClassroomGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.use_turn_level_advantage = args.use_turn_level_advantage
    ...
    def _build_message_token_spans(
        self, messages: List[Dict[str, Any]]
    ) -> List[Tuple[str, int, int]]:
        spans: List[Tuple[str, int, int]] = []
        prev_len = 0
        for i, message in enumerate(messages):
            prefix_token_ids = self.processing_class.apply_chat_template(
                messages[: i + 1],
                tokenize=True,
                add_generation_prompt=False,
            )
            curr_len = len(prefix_token_ids)
            spans.append((message.get("role", ""), prev_len, curr_len))
            prev_len = curr_len
        return spans

    def _compute_turn_group_advantages(
        self, rollout_metadata: List[Dict[str, Any]]
    ) -> Tuple[List[Dict[int, float]], Dict[str, float]]:
        grouped_turn_rewards: Dict[Tuple[int, int], List[Tuple[int, float]]] = defaultdict(list)
        per_rollout_advantages: List[Dict[int, float]] = [
            {} for _ in range(len(rollout_metadata))
        ]

        for rollout_idx, metadata in enumerate(rollout_metadata):
            problem_idx = metadata.get("problem_idx")
            turn_rewards = metadata.get("turn_rewards", {})

            if problem_idx is None or not isinstance(turn_rewards, dict):
                continue

            for turn_idx, reward in turn_rewards.items():
                if reward is None:
                    continue
                try:
                    turn_idx_int = int(turn_idx)
                    reward_float = float(reward)
                    problem_idx_int = int(problem_idx)
                except (TypeError, ValueError):
                    continue

                grouped_turn_rewards[(problem_idx_int, turn_idx_int)].append(
                    (rollout_idx, reward_float)
                )

        singleton_groups = 0
        for (_, turn_idx), members in grouped_turn_rewards.items():
            if len(members) == 1:
                singleton_groups += 1
                per_rollout_advantages[members[0][0]][turn_idx] = 0.0
                continue

            rewards = torch.tensor([reward for _, reward in members], dtype=torch.float32)
            mean_reward = rewards.mean().item()
            std_reward = rewards.std().item()

            for rollout_idx, reward in members:
                per_rollout_advantages[rollout_idx][turn_idx] = (
                    reward - mean_reward
                ) / (std_reward + 1e-4)

        group_sizes = [len(members) for members in grouped_turn_rewards.values()]
        stats = {
            "num_groups": float(len(group_sizes)),
            "singleton_groups": float(singleton_groups),
            "singleton_ratio": (
                float(singleton_groups) / float(len(group_sizes))
                if len(group_sizes) > 0
                else 0.0
            ),
            "avg_group_size": (
                float(sum(group_sizes)) / float(len(group_sizes))
                if len(group_sizes) > 0
                else 0.0
            ),
        }
        return per_rollout_advantages, stats

    def _build_token_level_turn_advantages(
        self,
        all_completion_ids: List[List[int]],
        rollout_metadata: List[Dict[str, Any]],
        per_rollout_advantages: List[Dict[int, float]],
    ) -> List[torch.Tensor]:
        token_level_advantages: List[torch.Tensor] = []

        for rollout_idx, token_ids in enumerate(all_completion_ids):
            seq_len = len(token_ids)
            token_advantages = torch.zeros(max(seq_len - 1, 0), dtype=torch.float32)

            if rollout_idx >= len(rollout_metadata):
                token_level_advantages.append(token_advantages)
                continue

            messages = rollout_metadata[rollout_idx].get("conversation_messages")
            if not isinstance(messages, list):
                token_level_advantages.append(token_advantages)
                continue

            spans = self._build_message_token_spans(messages)
            if len(spans) > 0 and spans[-1][2] != seq_len:
                logger.warning(
                    "Token span mismatch for rollout %s (metadata=%s, sampled=%s).",
                    rollout_idx,
                    spans[-1][2],
                    seq_len,
                )
            ...
            rollout_turn_advantages = per_rollout_advantages[rollout_idx]
            for role, start, end in spans:
                if role != "assistant":
                    continue

                advantage = rollout_turn_advantages.get(assistant_turn_idx, 0.0)
                if advantage != 0.0:
                    shifted_start = max(0, start - 1)
                    shifted_end = max(0, end - 1)
                    if shifted_start < token_advantages.numel():
                        token_advantages[
                            shifted_start : min(shifted_end, token_advantages.numel())
                        ] = advantage
    ...
    def _generate_and_score_completions(
        ...
        if self.accelerator.is_local_main_process:
            ...
            unique_problem_indices = list(range(len(ordered_set_of_prompts)))
            ...
            local_problem_indices = unique_problem_indices[start_slice:end_slice]
            ...
            all_rollout_metadata: List[Dict[str, Any]] = []
            sampled = sample_conversations(
                ordered_set_of_prompts,
                answers=real_answers,
                problem_indices=local_problem_indices,
                meta=meta_info_shared,
                server_port=self.server_port,
                num_samples_per_problem=self.num_generations,
                tokenizer=self.tokenizer,
                return_metadata=self.use_turn_level_advantage,
            )
            if self.use_turn_level_advantage:
                all_outputs, all_rollout_metadata = sampled
            else:
                all_outputs = sampled
        ...
        if self.use_turn_level_advantage:
            all_rollout_metadata = gather_object(all_rollout_metadata)
        ...
        turn_group_stats = None
        if self.use_turn_level_advantage:
            per_rollout_advantages, turn_group_stats = self._compute_turn_group_advantages(
                all_rollout_metadata
            )
            all_token_level_advantages = self._build_token_level_turn_advantages(
                all_completion_ids,
                all_rollout_metadata,
                per_rollout_advantages,
            )

            local_token_advantages = all_token_level_advantages[
                start_slice : start_slice + step_size
            ]
            advantages = torch.zeros(
                len(local_token_advantages), completion_ids.size(1) - 1, device=device
            )
            for i, token_adv in enumerate(local_token_advantages):
                if token_adv.numel() == 0:
                    continue
                limit = min(token_adv.numel(), completion_ids.size(1) - 1)
                advantages[i, :limit] = token_adv[:limit].to(device)

            rewards = advantages.mean(dim=1)
        ...
        elif turn_group_stats is not None:
            self._metrics[mode]["turn_groups/num_groups"].append(
                turn_group_stats["num_groups"]
            )
            self._metrics[mode]["turn_groups/singleton_groups"].append(
                turn_group_stats["singleton_groups"]
            )
            self._metrics[mode]["turn_groups/singleton_ratio"].append(
                turn_group_stats["singleton_ratio"]
            )
            self._metrics[mode]["turn_groups/avg_group_size"].append(
                turn_group_stats["avg_group_size"]
            )
        ...
        if self.accelerator.is_local_main_process:
            try:
                ...
                self._metrics[mode]["rewards/accuracy_reward"].append(
                    batch_metrics["accuracy_rewards"]
                )
        ...
        if (
            ...
            prompt_col = gather_object(prompts_text)
            completion_col = gather_object(completions_text)
            answer_col = gather_object([x["answer"] for x in inputs])
            reward_col = (
                gather_object(rewards.tolist())
                if self.use_turn_level_advantage
                else rewards.tolist()
            )

            min_len = min(
                len(prompt_col), len(completion_col), len(answer_col), len(reward_col)
            )
            if min_len == 0:
                logger.warning("Skipping completion table logging because one or more columns are empty.")
            else:
                if not (
                    len(prompt_col) == len(completion_col) == len(answer_col) == len(reward_col)
                ):
                    logger.warning(
                        "Completion table columns have mismatched lengths: "
                        f"prompt={len(prompt_col)}, completion={len(completion_col)}, "
                        f"answer={len(answer_col)}, reward={len(reward_col)}. "
                        f"Truncating to {min_len}."
                    )

            # For logging
                table = {
                    "prompt": prompt_col[:min_len],
                    "completion": completion_col[:min_len],
                    "answer": answer_col[:min_len],
                    "reward": reward_col[:min_len],
                }
    ...
    def compute_old_loss(
        ...
        if per_token_advantages.size(1) != per_token_logps.size(1):
            per_token_advantages = per_token_advantages[:, : per_token_logps.size(1)]
        ...
        if self.beta != 0.0:
            # Keep KL aligned with the same token support used by the loss.
            train_mask = completion_mask * assistant_mask
            train_denominator = train_mask.sum(dim=1).clamp_min(1)
            mean_kl = ((per_token_kl * train_mask).sum(dim=1) / train_denominator).mean()
            self._metrics[mode]["kl"].append(
                self.accelerator.gather_for_metrics(mean_kl).mean().item()
            )

            # Optional diagnostic: KL over all non-padding completion tokens.
            mean_kl_completion = (
                (per_token_kl * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)
            ).mean()
            self._metrics[mode]["kl_completion"].append(
                self.accelerator.gather_for_metrics(mean_kl_completion).mean().item()
            )
    ...
    def compute_loss(
        ...
        if self.use_liger_loss and inputs["advantages"].dim() == 2:
            logger.warning(
                "Per-token turn-level advantages are incompatible with Liger GRPO loss. Falling back to standard GRPO loss."
            )
