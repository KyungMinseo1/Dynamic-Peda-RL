...
class ParallelvLLMInference:
    def __init__(
        ...
        if original_visible_devices:
            self.original_gpu_ids = list(map(int, original_visible_devices.split(",")))
        else:
            self.original_gpu_ids = list(range(torch.cuda.device_count()))

        self.total_gpus = len(self.original_gpu_ids)
        ...
        print(f"[ParallelvLLMInference] original_gpu_ids={self.original_gpu_ids}")
