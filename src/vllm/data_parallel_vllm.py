...
def _worker_loop_fn(gpu_group, task_queue, result_queue, config):
    ...
    while True:
        ...
        if inference_task == InferenceTask.REWARD:
            ...
            # The caller already formats prompts with apply_chat_template(), so
            # use text generation directly instead of re-applying chat formatting.
            outs = llm.generate(
                prompts,
...
class ParallelvLLMInference:
    ...
    def run_batch(
        ...
        if self.n_instances <= 0:
            raise RuntimeError(
                f"No vLLM instances available (n_instances={self.n_instances}). "
                "Check GPU allocation and initialization."
            )
        ...
        # Only use as many workers as we have messages, up to n_instances
        num_workers_to_use = min(self.n_instances, total)
        chunk_size = math.ceil(total / num_workers_to_use)
        chunks = [
        ...
        while received < expected:
            # Only check the queues of workers that were actually given a task
            for i in range(num_workers_to_use):
