            ...
            conversation.append(
                {
                    "role": "user",
                    "content": Template(
                        self.student_final_prompt
                    ).render(problem=self.problem)
                }
            )
            return conversation
            """
...
class Classroom:
    ...
    def get_pedagogical_alignment_reward(self, conversation: Conversation):
        ...
        if len(conversation.constructivist_failed_judges) == 0:
            # No judges evaluated; return neutral reward
            return reward
    ...
    def judge_conversations_with_paths(
        self,
        conversations: List[Conversation],
        prompts_paths: Dict[str, str],
        store_in_constructivist: bool = False,
    ):
        import json
        from jinja2 import Template
        ...
        for checklist_name, prompt_path in prompts_paths.items():
            pending_convs = [
                c for c in conversations
                if checklist_name not in c.judge_decisions
            ]
            if not pending_convs:
                continue

            logger.info(f"Judging [{checklist_name}] for {len(pending_convs)} conversations")
            valid_responses = {conv: [] for conv in pending_convs}

            for _round in range(max_rounds):
                pending = []
                for conv in pending_convs:
                    missing = num_attempts - len(valid_responses[conv])
                    if missing > 0:
                        message = [
                            {
                                "role": "user",
                                "content": Template(open(prompt_path).read()).render(
                                    conversation=conv._get_hidden_conversation()
                                ),
                            }
                        ]
                        for _ in range(missing):
                            pending.append((conv, message))

                if not pending:
                    break

                responses = self.judge_model.run_batch(
                    [msg for _, msg in pending], self.sampling_params_judge
                )
                for (conv, _), response in zip(pending, responses):
                    for output in response.outputs:
                        try:
            ...
            for conv in pending_convs:
                while len(valid_responses[conv]) < num_attempts:
                    valid_responses[conv].append(
                        JudgeResponse(reasoning="max rounds exceeded", decision="OK")
                    )
                decisions = valid_responses[conv]
                conv.judge_decisions[checklist_name] = decisions

                if store_in_constructivist:
                    is_failed = any(d.decision == JudgeDecision.REJECT for d in decisions)
                    conv.constructivist_failed_judges[checklist_name] = is_failed

        self.judge_model.sleep()

    def save_initial_attempts(self, conversations: List[Conversation], save_path: str):
        import os, json
        data = {}
        for conv in conversations:
            if conv.initial_attempts:
                data[conv.problem] = {
                    "initial_attempts": conv.initial_attempts,
                    "initial_rewards": conv.initial_rewards,
                }
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        with open(save_path, "w") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        logger.info(f"Saved initial attempts to {save_path} ({len(data)} problems)")


    def load_initial_attempts(self, conversations: List[Conversation], load_path: str) -> bool:
        import os, json
        if not os.path.exists(load_path):
            logger.info(f"No cached initial attempts found at {load_path}")
            return False
        ...
        hit, miss = 0, 0
        for conv in conversations:
            if conv.problem in data:
                conv.initial_attempts = data[conv.problem]["initial_attempts"]
                conv.initial_rewards  = data[conv.problem]["initial_rewards"]
                hit += 1
            else:
                miss += 1

        logger.info(f"Loaded initial attempts: hit={hit}, miss={miss}")
        return miss == 0  # 전부 캐시 히트하면 True